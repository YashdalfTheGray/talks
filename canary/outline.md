...
Each of these headings generally refer to a group of slides, slide (and other) breaks are noted.
...
I work for a large cloud provider. My team manages a hosted docker service console. The details of the team aren't really all that important, the only important tidbit is that it's a relatively complex webapp with integrations to many other services!
...
- Console - My team uses the word `console` to mean the web interface to my service. This term is interchangable with `web app`.
- Region - We is a regionalized service which means that we have data centers all over the world and you can choose to run your service in the one closest to you. We are currently exists in 18 regions; this will become important later in our story.
...
- There are integration (or end-to-end) testing tools that exist that my team uses but we often don’t get enough time to develop testing around all of the user stories that we should be testing for.
...
- Another dimension where this idea doesn’t work is scale. My service is all about massive scale. We test, by hand, deployments to 18 regions; this would take anywhere from 45 minutes to an hour per region over 5 days. That's half a developer week, gone.
...
- Furthermore, if I can run a project on Docker, I can run it on my service and other hosted Docker services as well!
...
- **Slide break**
- So now I had Jest, Puppeteer, and Docker! I thought, "Excellent, let's get started". So I took some time to write tests against the production code and found that I could write tests using my stack a lot faster than I could using the internal integration testing stack.
...
- What I want to do, and it is related to the second part of my talk - canaries. I don't just want to find potential issues when I'm looking for it, given that we've created a significant amount of integration tests for our app, I want them to run constantly and break and tell me when there has been an error that we might have missed or when there is a dependency of our web app that isn't working.
- **Slide break**
- This is what's called a canary. It's a set of forever running tests that access and walk through your application to catch issues potentially before any customer does. Canaries are a pretty standard thing for us, all services and consoles are required to have them.
- The word comes from the phrase "canary in the coal mine". Coal miners would keep a canary bird around to detect poisonous gasses. When the canary stopped singing, it was time to leave the mine. Similarly, when our digital canary stops singing (working as expected), we know something's wrong.
...
- The idea to take this further is simple, integrate with something like Amazon CloudWatch Events and create events for each success and failure and have the failure email you that something's wrong.
...
- **KitKat break**
- This is one of the key takeaways today, the whole stack, Chromium + Jest + Puppeteer runs on Docker which can then be run pretty much anywhere. It's truly, write once, run anywhere code. As an example, I can run this suite of tests on my personal computer, on my work computer, across my team's computers, on our servers and even on the cloud's hosted container offerings. All so that I know that the code that I'm shipping is doing what I intended it to and that outages in my service aren't being communicated to me by my customers but by my canary. Happy developers, happy customers.
- **Slide break**
- The last part of the stack focuses on the canary piece. We want these tests to run constantly, over and over and inform us of any issues that happen when something goes, not as planned. We already have the tests, we can use something like a cronjob or a process manager to run these things over and over again.
- The notification system is also simple. We can integrate the tests with Amazon CloudWatch Events, every time a test runs, we fire off an event that says what platform it was running on and what was the result. We can then use an Amazon SNS topic as the target of the CloudWatch Event to send us emails every time something fails and where the failure happened, whether in preprod, staging or prod.
